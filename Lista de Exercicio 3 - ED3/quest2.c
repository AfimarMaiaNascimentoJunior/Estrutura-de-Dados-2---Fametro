...
void estatisticasFila(Fila *f) {
    if (filaVazia(f)) {
        printf("A fila esta vazia. Nao ha estatisticas.\n");
        return;
    }
    ...
    // Esvazia f, calcula e guarda em aux
    while (!filaVazia(f)) {
        int valor = dequeue(f);
...
void transfereF1paraF2(Fila *f1, Fila *f2) {
    ...
    while (!filaVazia(f1)) {
        ...
        enqueue(&aux, valor);
        enqueue(f2, valor); // Copia para F2
    }
    ...
    printf("Elementos de F1 transferidos (copiados) para F2.\n");
}
